...
- Flow: panel button (`thgl:ticket:open`) → modal (subject/description/optional game text) → private thread with opener mention + **staff-role mention** (`TICKET_STAFF_ROLE_ID` — adds and notifies the whole team; owner decision 2026-08-29: pings are back, since one-by-one `thread.members.add` pinged staff anyway; staff who dislike pings mute the panel channel). A reply reopening a closed thread re-pings staff unless the replier is staff. Staff also see all threads via ManageThreads, and the log channel records every event. (GuildMembers intent still unused — needs Discord verification review at this scale; irrelevant now that the role mention is back.)
- **Per-game embed colors** (staff idea, approved 2026-08-29): `gameColor()` in `lib/tickets.ts` maps the free-text Game/App field to a stable color (matched to a game slug via `GAME_CONFIGS` titleKeywords, else hashed raw text) — applied to the ticket embed and the log-channel embeds so `#🎫・ticket-log` is scannable by game at a glance. No game given → default green. Optional free-text Game/App field is shown in the ticket embed. Close button archives; a user reply to a closed thread auto-unarchives it (no staff re-ping). Inactivity scheduler warns then auto-closes (`TICKET_*` env vars, see README).
...
- Reactive delete+ban rules (mod log: `MOD_LOG_CHANNEL_ID`, prod = `#bot-log` 1473951897353457736): Rule 1 cross-channel image spam (>=3 images across >=2 channels/60s), Rule 2 rapid multi-channel posting (>=4 channels/30s), **Rule 0** instant single-message signatures (>=3 images with no text, or a bare Discord invite link). Bans use `deleteMessageSeconds` (1h server-side purge) PLUS a gateway sweep: any message from an already-flagged user is deleted on arrival - Discord's purge misses messages committed in the same instant as the ban (audit-log-confirmed 2026-08-30; that was the recurring "one leftover message" staff removed by hand, usually in the campaign's third channel).
- **Rule 0 gating** (`rule0Decision`, pure + asserted in `scripts/test-spam-guard.ts`): the signature is the same for everyone, three gates decide who may be acted on. (1) **new member** (joined <7 days) - acts on its own, the original proven gate. Every other gate additionally requires that the account has **never posted**, so an established member who talks here is never touched whatever their roles are. (2) **no onboarding role** + never posted -> ban: onboarding's prompt is required, so an account that picked nothing is not a real member. The role set is `getSelfAssignableRoleIds()` (every onboarding-grantable role - games AND non-game options like Coding/Development - plus every game role resolved by canonical title and every hardcoded `roleIds`), refreshed every 10 min; if the set is empty or there is no member object the gate downgrades to review, so missing data can never ban. (3) **review**: onboarded, established, but silent until now = the hijacked-account shape -> recorded only while `RULE0_HISTORY_LEG = "log"`; flip to `"ban"` once the mod log proves these are never real members. "Never posted" = a miss in the in-memory `knownPosters` map (every human message incl. exempt channels, 30d TTL) AND no message from them in the channel's last 100, because the map is empty after every restart.
- **Honeypot ordering (root-caused 2026-09-13)**: the campaigns enumerate by Discord's raw `position` field - a per-category sort key, so sorting by it globally ignores categories. `sortAppsAndGamesChannels` renumbers Apps & Games children to 0..N, which puts `#thgl-companion-app` at raw 2, while the trap (created last in the top category, which owns 9..11) sat at raw 11 = 10th writable and was never reached. The mod log matches exactly: 2-channel campaigns hit the first 2 writable by raw position, the 2026-08-26 invite spray hit the first 4, in order. Fixed with `scripts/fix-trap-position.ts` (sets the trap to raw 0; snapshots, verifies, `--revert`). **Keep the trap at raw 0 or 1** - Apps & Games always restarts at 0. Minority kits enumerate differently: unsorted gateway order (soulmask+crimson-desert) and channel-ID order (new-world-map+skeleton). Earlier theories (hardcoded targets, onboarding visibility) are superseded: the trap always had `@everyone` ViewChannel, and adding it to onboarding defaults on 2026-08-31 changed nothing.
- **Honeypot** `#🪤・bot-trap` (`TRAP_CHANNEL_ID`, default 1542957161494093909, created by `scripts/setup-trap-channel.ts`): now the first writable channel in BOTH display order and raw-position order. Any post there is deleted; only posts with attachments/links (the bot signature) get banned - plain text is logged with **no action**, so a confused human is never punished.
- **Mod-log forensics**: every detection embed carries account age + join age, all roles and which are game roles, the attachment names/sizes/dimensions (campaigns reuse the same files across accounts), and for each channel hit its raw position plus its rank in BOTH the raw and display orderings - that is what identifies which enumeration a campaign used. If Discord rejects the embed, `sendModLog` re-sends it as flattened plain text so evidence is never dropped.
- **Scoreboard**: a pinned counter embed in the trap channel (footer marker `thgl:trap:counter`, the message IS the store) counts total bans + trap catches; `bumpSpamCounter` increments it serialized after each successful ban. Seeded 2026-08-30 from full mod-log history (201 bans).
- Bots (incl. this one) and `SAFE_ROLE_IDS`/staff are excluded before ANY rule or trap logic - they can never be actioned.
- **Exempt channels** (`EXEMPT_CHANNEL_IDS`, extend via `SPAM_GUARD_EXEMPT_CHANNEL_IDS`): support-ticket (+ its private threads), suggestions-issues (+ forum posts) and the legacy MEE6 ticket categories are skipped entirely - no tracking, no rules, not even the flagged-user sweep. Ids may name a channel, forum, or category; `isExemptChannel` walks thread -> parent channel -> category. Added 2026-09-11 after Rule 0 banned a real user for posting screenshots in their own ticket thread (staff report 2026-09-10).
...
- `scripts/setup-trap-channel.ts` - Spam guard: create the honeypot channel (idempotent)
- `scripts/fix-trap-position.ts` - Spam guard: keep the honeypot at raw position 0 so it wins the ordering the bots enumerate (dry-run by default, `--apply`, `--revert <snapshot>`)
- `scripts/test-spam-guard.ts` - Spam guard: Rule 0 decision-table assertions (no token needed)
